...
// gs mock -o gs_mock.go -i 'CondContext,ArgContext,Runner,Job,Server'
...
type CondContextMockImpl struct {
	r *gsmock.Manager
}

func NewCondContextMockImpl(r *gsmock.Manager) *CondContextMockImpl {
	return &CondContextMockImpl{r: r}
}

func (impl *CondContextMockImpl) Has(key string) bool {
	t := reflect.TypeFor[CondContextMockImpl]()
...
func (impl *CondContextMockImpl) MockHas() *gsmock.Mocker11[string, bool] {
	t := reflect.TypeFor[CondContextMockImpl]()
	return gsmock.NewMocker11[string, bool](impl.r, t, "Has")
}

func (impl *CondContextMockImpl) Prop(key string, def ...string) string {
	t := reflect.TypeFor[CondContextMockImpl]()
...
func (impl *CondContextMockImpl) MockProp() *gsmock.Mocker21[string, []string, string] {
	t := reflect.TypeFor[CondContextMockImpl]()
	return gsmock.NewMocker21[string, []string, string](impl.r, t, "Prop")
}

func (impl *CondContextMockImpl) Find(s BeanSelector) ([]CondBean, error) {
	t := reflect.TypeFor[CondContextMockImpl]()
	if ret, ok := gsmock.Invoke(impl.r, t, "Find", s); ok {
		return gsmock.Unbox2[[]CondBean, error](ret)
	}
	panic("no mock code matched")
}

func (impl *CondContextMockImpl) MockFind() *gsmock.Mocker12[BeanSelector, []CondBean, error] {
	t := reflect.TypeFor[CondContextMockImpl]()
	return gsmock.NewMocker12[BeanSelector, []CondBean, error](impl.r, t, "Find")
}
