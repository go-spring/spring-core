...
func TestApplicationContext_RegisterBeanFn2(t *testing.T) {
	...
	t.Run("manager", func(t *testing.T) {
		...
		assert.Equal(t, bd.B.BeanName(), "NewManager")
...
func TestApplicationContext_Collect(t *testing.T) {
	...
	t.Run("", func(t *testing.T) {
		...
		assert.Equal(t, intBean.B.BeanName(), "TestApplicationContext_Collect.func6.1")
...
func TestApplicationContext_RegisterMethodBean(t *testing.T) {
	...
	t.Run("method bean", func(t *testing.T) {
		...
		bd := c.Provide((*Server).Consumer, parent.B)
		...
		assert.Nil(t, err)
		assert.Equal(t, bd.B.BeanName(), "Consumer")
	})
	...
	t.Run("method bean arg", func(t *testing.T) {
		...
		c := gs_core.New()
		parent := c.Object(new(Server))
		c.Provide((*Server).ConsumerArg, parent.B, "${i:=9}")
	...
	t.Run("method bean wire to other bean", func(t *testing.T) {
		...
		c.Provide(ServerInterface.Consumer, parent.B.ID()).DependsOn("ServerInterface")
	...
	t.Run("circle autowire", func(t *testing.T) {
		...
		for i := 0; i < 20; i++ { // 不要排序
			func() {
				...
				c.Provide((*Server).Consumer, parent.B.ID()).DependsOn("Server")
				c.Object(new(Service))
				c.RefreshProperties(prop)
				err := c.Refresh()
				if err != nil {
...
func TestDefaultSpringContext(t *testing.T) {
	...
	t.Run("method bean cond", func(t *testing.T) {
		...
		c := gs_core.New()
		parent := c.Object(new(Server))
		c.Provide((*Server).Consumer, parent.B.ID()).On(gs_cond.OnProperty("consumer.enable"))
